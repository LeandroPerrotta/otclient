UICEFWebView = extends(UIWidget, "UICEFWebView")

function UICEFWebView.create()
    local webView = UICEFWebView.internalCreate()
...
function UICEFWebView:loadComponent(component_name, component_path)
    ...
    for _, css_file in ipairs(css_file_list) do
        local css_path = absolutePathFn(css_file)
    ...
    for _, js_file in ipairs(js_file_list) do
        local js_path = absolutePathFn(js_file)
